...
window.renderDashboard = function() {
  ...
  if (window.currentClass !== selectedRoom) {
    window.currentClass = selectedRoom;
  ...
  let behSum = 0;
  sts.forEach(s => {
    behSum += (s.behaviorScore !== undefined ? s.behaviorScore : 0);
  });
  ...
  // 6. Behavior Risk < 0 count
  let behRiskCount = 0;
  sts.forEach(s => {
    const beh = (s.behaviorScore !== undefined ? s.behaviorScore : 0);
    if (beh < 0) {
      behRiskCount++;
    }
  });
        ...
        return `
          <div style="margin-bottom:10px;">
            <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px;">
              <span style="font-weight:500;">${s.no}. ${window.esc(s.name)} ${selectedRoom === 'all' && s.roomName ? `<span style="font-size:10px;color:var(--text3)">(${window.esc(s.roomName)})</span>` : ''}</span>
  ...
  if (sts.length > 0) {
    sts.forEach(s => {
      ...
      if (behScore < 0) {
        reasons.push(`คะแนนพฤติกรรมติดลบ (${behScore} คะแนน)`);
        if (behScore <= -10) isHighRisk = true;
      }
          ...
          <div class="risk-item">
            ...
            <div style="flex:1;">
              <div style="font-size:13px;font-weight:600;display:flex;align-items:center;gap:6px;flex-wrap:wrap">
                <span>${s.no}. ${window.esc(s.name)}</span>
                ${selectedRoom === 'all' && s.roomName ? `<span style="font-size:11px;color:var(--text3);font-weight:normal">(${window.esc(s.roomName)})</span>` : ''}
  ...
  if (topListEl) {
    ...
    const sortedByBeh = [...sts].sort((a, b) => {
      const behA = a.behaviorScore !== undefined ? a.behaviorScore : 0;
      const behB = b.behaviorScore !== undefined ? b.behaviorScore : 0;
      return behB - behA;
    });
          ...
          <div style="display:flex;align-items:center;padding:10px 12px;border-radius:10px;margin-bottom:8px;background:${bgColors[idx]||'var(--surface2)'};border:1px solid ${borderColors[idx]||'var(--border)'};gap:10px;transition:transform .15s">
            ...
            <div style="flex:1;display:flex;align-items:center;gap:10px;min-width:0;">
              <span class="avatar" style="width:30px;height:30px;font-size:9px;background:var(--accent-light);color:var(--accent);display:flex;align-items:center;justify-content:center;border-radius:50%;font-weight:600;flex-shrink:0;box-shadow:0 1px 2px rgba(0,0,0,0.05)">
                ${window.esc(window.initials ? window.initials(s.name) : s.name.charAt(0))}
              </span>
              <div style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                <div style="font-size:12px;font-weight:600;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${window.esc(s.name)}</div>
                <div style="font-size:10px;color:var(--text3);margin-top:2px;">
                  เลขที่ ${s.no} ${selectedRoom === 'all' && s.roomName ? `· ห้อง ${window.esc(s.roomName)}` : ''}
                </div>
              </div>
            </div>
