...
window.renderBehaviorPanel = function() {
  ...
  if (rsel) {
    let options = `<option value="all" ${window.currentClass === 'all' ? 'selected' : ''}>🌟 ทุกห้องเรียน</option>`;
...
window.renderBehaviorGrid = function() {
  ...
  if (activeRoom === 'all') {
    ...
    students = (window.classData[activeRoom] || []).map(s => Object.assign({}, s, { roomId: activeRoom }));
  }
  ...
  el.innerHTML = students.map(s => {
    ...
    const roomBadge = activeRoom === 'all' && s.roomName ? ` <span class="badge badge-info" style="font-size:10px;padding:2px 6px;">${window.esc(s.roomName)}</span>` : '';
    ...
    return `
      <div class="beh-student-card">
        ...
        <h4>${window.esc(s.name)}${roomBadge}</h4>
...
window.openAwardModal = function(studentId, type) {
  ...
  let students = [];
  if (activeRoom === 'all') {
    Object.values(window.classData).forEach(list => {
      students = students.concat(list);
    });
  } else {
    students = window.classData[activeRoom] || [];
  }
  ...
  if (subtitle) {
    ...
    subtitle.innerHTML = `นักเรียน: <strong>${window.esc(student.name)}</strong> (${rollStr} · ID: ${student.id})`;
  }
...
window.submitBehaviorPoints = function() {
  ...
  let sRoomId = '';
  if (activeRoom === 'all') {
    for (const [rid, list] of Object.entries(window.classData)) {
      s = list.find(x => x.id === studentId);
      if (s) {
        sRoomId = rid;
        break;
      }
  ...
  const feedbackMsg = type === 'pos' 
    ? `✅ มอบแต้มความดีให้ ${window.esc(s.name)} +${pts} คะแนนสำเร็จ (${scoreAfter} แต้ม)` 
    : `⚠️ หักคะแนนความประพฤติ ${window.esc(s.name)} -${pts} คะแนนสำเร็จ (${scoreAfter} แต้ม)`;
  window.toast(feedbackMsg);

  // Re-render panels
  window.renderBehaviorPanel();
};
...
window.renderBehaviorLeaderboard = function() {
  ...
  if (activeRoom === 'all') {
    window.rooms.forEach(r => {
      students = students.concat(window.classData[r.id] || []);
    });
  } else {
    students = window.classData[activeRoom] || [];
  }
  ...
  const sorted = [...students].map(s => ({
    ...s,
    score: s.behaviorScore !== undefined ? s.behaviorScore : 0
    ...
    return `
      <div class="beh-leader-row">
        ...
        <div style="flex: 1; min-width: 0; font-size: 13px; font-weight: 600; color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: flex; align-items: center;">
          <span style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1;">${window.esc(s.name)}</span>
...
window.renderBehaviorHighlights = function() {
  ...
  const logs = (window.behaviors || []).filter(b => (activeRoom === 'all' || b.cls === activeRoom) && (currentTerm === 'all' || !b.term || b.term === 'all' || b.term === currentTerm));
