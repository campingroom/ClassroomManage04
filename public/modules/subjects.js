...
window.renderSubjectList = function(){
  ...
  // Update subject list title count
  const title = document.getElementById('subject-card-title');
  if (title) {
    title.innerHTML = `รายวิชาทั้งหมด <span style="background:var(--accent-light);color:var(--accent);padding:2px 8px;border-radius:12px;font-size:12.5px;margin-left:6px;">${filtered.length}</span>`;
  }
  ...
  let rowsHTML = filtered.length ? filtered.map(s=>{
    ...
    const dot = `<span class="subject-dot" style="background: ${color}"></span>`;

    return `<tr>
      <td style="font-size:12px;color:var(--text3);font-family:monospace;font-weight:600">${s.code||'-'}</td>
      <td style="font-weight:600">${dot}${window.esc(s.name)}${typeBadge}</td>
      <td style="font-size:12px;color:var(--text2)">${termText}</td>
      <td style="font-size:12px; display:none">${window.esc(s.teacher||'-')}</td>
      <td class="ctr"><span class="badge badge-info">${s.credits}</span></td>
      <td style="font-size:12px;color:var(--text2)">${roomNames||'-'}</td>
      <td class="ctr">
        <div style="display:flex;gap:6px;justify-content:center">
          <button class="btn-edit-outlined" onclick="editSubject('${s.id}')" style="padding: 4px 10px; font-size: 11.5px;">✏️ แก้ไข</button>
          <button class="btn-delete-outlined" onclick="deleteSubject2('${s.id}')" style="padding: 4px 10px; font-size: 11.5px;">ลบ</button>
        </div>
      </td>
    </tr>`;
  ...
  if (filtered.length) {
    rowsHTML += `<tr style="background:var(--surface2);font-weight:700">
      <td colspan="3" style="text-align:right;vertical-align:middle">หน่วยกิตรวมสะสม:</td>
...
window.clearSubjForm = function(){
  ...
  ss('subj-code','');
  ss('subj-credits','2');
  ss('subj-teacher', window.teacherName || 'ครูผู้สอน');
  ss('subj-note','');
  ss('subj-term','1');
  ss('subj-type','basic');
...
window.saveSubject = function(){
  ...
  const teacher = document.getElementById('subj-teacher')?.value || window.teacherName || 'ครูผู้สอน';
...
window.editSubject = function(id){
  ...
  ss('subj-teacher', s.teacher || window.teacherName || 'ครูผู้สอน');
  ss('subj-note', s.note||'');
