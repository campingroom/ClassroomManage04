      ...
      <td><div style="display:flex;align-items:center;gap:8px">
        ...
        <div>
          ...
          <div style="display:flex;gap:6px;align-items:center;margin-top:2px">
            ${s.roomName ? `<span style="background:var(--accent-light);color:var(--accent);font-size:10px;padding:1px 6px;border-radius:4px;font-weight:600">ห้อง ${s.roomName}</span>` : ''}
            ${s.health && s.health !== '-' ? `<span style="font-size:11px;color:var(--text3)">ชื่อเล่น: ${s.health}</span>` : ''}
          </div>
        </div>
      </div></td>
      <td style="font-size:12px;color:var(--text2)">${s.gender}</td>
      <td style="font-size:12px;color:var(--text2)">${s.phone || '-'}</td>
      <td style="font-size:12px;color:var(--text2)">${s.health || '-'}</td>
...
window.exportStudentCSV = function () {
  ...
  sts.forEach(s => {
    ...
    csv += `${s.no},"${s.name}",${s.gender},"${roomVal}",${s.dob || ''},${s.phone || ''},"${s.addr || ''}","${s.health || '-'}",${t > 0 ? t : '-'},${t > 0 ? window.getGrade(t) : '-'}\n`;
  });
...
window.openStudentProfileModal = function (studentId, initialTab) {
  ...
  if (nameEl) {
    ...
    nameEl.innerHTML = `${student.name}${nickname ? `<div style="font-size:13px;font-weight:600;color:var(--text3);margin-top:2px">ชื่อเล่น: ${nickname}</div>` : ''}`;
  }
  ...
  if (detEl) {
    ...
    detEl.innerHTML = `เลขประจำตัว: ${student.id} • ห้องเรียน: ${roomStr}`;
  }
...
window.renderStdProfileBehaviors = function () {
  ...
  panel.innerHTML = logs.map(b => {
    ...
    return `
      <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 14px; border-bottom: 1px solid var(--border); font-size: 13px;">
        <div>
          <div style="font-weight: 600; color: var(--text);">${b.note}</div>
...
window.renderStudentProfilePage = function () {
  ...
  // 1. Populate room dropdown if empty
  const currentRoomVal = roomSelect.value;
  if (!roomSelect.options.length && window.rooms && window.rooms.length > 0) {
    roomSelect.innerHTML = (window.rooms || []).map(r => {
      return `<option value="${r.id}">${r.level}/${r.section}</option>`;
    }).join('');

    if (currentRoomVal && (window.rooms || []).some(r => r.id === currentRoomVal)) {
      roomSelect.value = currentRoomVal;
    } else {
      roomSelect.value = window.rooms[0].id;
    }
  }
  ...
  gridEl.innerHTML = filtered.map(s => {
    ...
    if (behScore < 100) {
      ...
      behCol = 'var(--red)';
    } else if (behScore === 100) {
      behBg = 'var(--surface2)';
      behCol = 'var(--text2)';
    }

    // Attendance rate
    const sAttRecs = Object.entries(window.attData).filter(([k]) => k.startsWith(roomId + '_' + s.id + '_'));
    const sN = sAttRecs.length;
    const sP = sAttRecs.filter(([, v]) => v === 'P' || v === 'L').length;
    const attPct = sN > 0 ? Math.round((sP / sN) * 100) : 100;
    ...
    return `
      <div class="card profile-grid-card" onclick="window.openStudentProfileModal(${s.id})" 
        ...
        <!-- Name & ID -->
        <h3 style="font-size: 15px; font-weight: 800; margin: 0 0 6px 0; color: var(--text); line-height: 1.4;">${s.name}${nickStr}</h3>
